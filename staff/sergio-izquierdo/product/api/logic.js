...
class Logic {
	...
	changeUserPassword(userId, password, newPassword, newPasswordRepeat) {
        ...
        if(typeof newPasswordRepeat !== 'string') throw new ValidationError('invalid email type')
        if(newPasswordRepeat.length < 8) throw new ValidationError('invalid newPasswordRepeat length')

		if(newPassword !== newPasswordRepeat) throw new ValidationError('newPassword and nePasswordRepeat do not match')
